Fixed and Growth Mindsets | May 22

Quiz 1 topics to cover:
- look into research approaches
- Cattell's 16 personality factors
- trait approaches: single-trait, typological, psychodynamic, many-trait
	- what type is myers-briggs? -typological
	- single-trait:
	- typological: categorization into types or categories
	- psychodynamic:
	- many-trait:
- explanations for exceptions in behaviour that deviate from general pattern
	- e.g. Maya feeling confident talking to strangers at a party but being terrified to public speak
- personality can predict behaviour across situations: person x situation interaction 

Lecture 1 recap
Normal distribution of traits
- many modern personality theories reflect an origin in the study and treatment of "disturbed" behaviour
- continuity assumption: assumption there is no discontinuity between "disturbed"...
- differences in degree, not in kind , between normal and abnormal behaviour
	- differences in level of trait change outcome of behaviour - high anxiety > avoid studying, moderate anxiety > motivate to study
	- analysis of trait across people
- cross-situational consistency: the same person demonstrating similarity across situations
	- the same quality that is the same in a person across situations
	- most personality traits are fairly stable across situations
- person-situation interaction
	- the same quality that is different in a person across different situations

Lecture 2: Growth and Fixed Mindsets
- implicit theories: refers to someone's personal ideas, beliefs about the self and social world
	- implicit beliefs guide how people think about themselves, interpret events and make attributions, and how they respond to important and benign experiences (how we choose to behave in the future)
- types of implicit beliefs
	- entity theory/fixed mindset: belief a particular attribute is immutable
	- whereas incremental theory/growth mindset reflects the assumption that a particular attribute is subject to change
- fixed mindset: belief intelligence is fixed, each person has their allotted share, and there is little they can do to increase that allotment
- whereas growth mindset reflects the belief that intelligence is modifiable through **increased effort**
- growth mindset associated with upward trajectory in grades, stable mindset associated with flat trajectory
fixed vs growth mindset
- different goals
	- adoption of growth mindset leads to a preference of challenging tasks, which increases their ability and helps them achieve greater mastery
		- encourages people to respond to learning opportunities to learn something new
	- fixed mindset > often leads to performing actions in order to **appear** capable and avoid challenge - primary concern is with being perceived as adequately capable, and failure would convey the opposite (performance goals)
- attribution to failure
	- individuals who subscribe to an incremental view will attribute a failure to not yet having learned something, looking at something from the incorrect perspective, or not working hard enough
	- all these problems can be corrected through effort, leading incrementalists to continually seek any situation that will intellectually better themselves
	- more likely to engage in remedial action to correct mistakes if necessary
	- fixed intelligence views attribute failure to their own lack of ability
- response to challenges
	- growth mindset: mistakes are seen as opportunities to learn and improve
	- fixed mindset: mistakes are seen as a lack of ability > leads to avoiding seeking help
	- 
- self-regulated learning
	- 
